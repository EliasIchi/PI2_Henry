...
st.subheader("WordCloud de Direcciones Normalizadas")

# Cargar los datos
url_homicidios = "https://cdn.buenosaires.gob.ar/datosabiertos/datasets/transporte-y-obras-publicas/victimas-siniestros-viales/homicidios.xlsx"
h_hechos = pd.read_excel(url_homicidios)

# Filtrar valores no nulos en la columna "Dirección Normalizada"
direccion_normalizada = h_hechos['Dirección Normalizada'].dropna()

# Concatenar todas las listas generadas por el método .str.split()
words_list = direccion_normalizada.str.split().explode()
...
st.subheader("WordCloud de dataset hechos homicidios")
...
text_series = h_hechos.astype(str).stack().reset_index(drop=True)

# Concatenar todas las palabras en una sola cadena de texto
...
st.image(wordcloud.to_array(), caption='WordCloud de todo el DataFrame')